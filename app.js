...
function computeReport() {
  ...
  const baseCost    = EDU_COSTS[goal]?.base ?? 800;
  const targetCost  = Math.round(baseCost * Math.pow(1 + INFLATION_RATE, yearsLeft));
  ...
  // Score (0–100)
  let score = 100;
  const coverageRatio = projectedSavings / targetCost;
  score = Math.min(100, Math.round(coverageRatio * 100));
  ...
  // Countdown
  const now          = new Date();
  const univYear     = now.getFullYear() + yearsLeft;
  const univDate     = new Date(univYear, 8, 1); // 1 Sep
  ...
  State.report = {
    score, scoreDesc, targetCost, projectedSavings, gap,
    cYears, cMonths, cDays, univYear, yearsLeft, goal,
  };
}
...
async function submitLead() {
  ...
  try {
    ...
    // Fail gracefully: still thank the user
